...
from queue import Queue, Empty  # Imported Empty properly
from threading import Thread, Event
import csv
import gc
from urllib.parse import urlparse
import argparse  # Fixed the import (was 'argeparse')
...
def get_config_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--csv-path',
                        type=str,
                        default=os.environ.get('CSV_PATH', 'big_production_sampling.csv'),
                        help='Path to CSV containing image URLs')
    parser.add_argument('--results-dir',
                        type=str,
                        default=os.environ.get('RESULTS_DIR', 'processing_results'),
...
class AsyncImageLoader:
    ...
    def __init__(self, csv_path, batch_size=8, max_prefetch=3, num_workers=8, temp_dir='temp_downloads'):
        """
    ...
    def download_image(self, url, idx):
        ...
        try:
            ...
            save_path = os.path.join(self.temp_dir, basename)
...
def load_models(device, dino_model_id, sam2_model_cfg, sam2_checkpoint):
...
def upload_to_s3(s3_client, file_path, bucket, s3_key):
...
def load_and_preprocess_image(metadata, image_size):
...
def process_images(csv_path, results_dir, results_csv, final_results_csv, args):
    """
    ...
    Args:
        ...
        results_csv: Path to save intermediate results CSV
        final_results_csv: Path to save final results CSV
    ...
    loader = AsyncImageLoader(
        ...
        max_prefetch=args.max_prefetch_batches,
    ...
    try:
        ...
        for batch_idx, batch_metadata in tqdm(batch_generator, desc="Processing batches", total=loader.num_batches):
            ...
            batch_data = [load_and_preprocess_image(metadata, args.image_size) for metadata in batch_metadata]
            batch_images = [img for img, _ in batch_data]
            batch_meta = [meta for _, meta in batch_data]
            current_batch_files = {meta['path'] for meta in batch_meta}
            processed_files.update(current_batch_files)
            # Skip empty batch
            if not batch_images:
                continue
                
            # Process with DINO
            text_queries = [[args.text_query]] * len(batch_images)
            ...
            dino_results = dino_processor.post_process_grounded_object_detection(
                outputs,
                inputs.input_ids,
                threshold=args.dino_box_threshold,
            ...
            for j, (image, meta, result) in enumerate(zip(batch_images, batch_meta, dino_results)):
                ...
                if dino_score >= args.dino_box_threshold and box_area_pct < 0.8:
                    # Prepare box for SAM
                    ...
                    if sam_score >= args.sam_score_threshold:
                        ...
                        # Upload to S3
                        try:
                            s3_sample_key = f"{args.s3_samples_prefix}{basename}"
                            s3_mask_key = f"{args.s3_masks_prefix}{basename_no_ext}_binarymask.jpg"
                            
                            upload_to_s3(s3_client, result_image_path, args.s3_bucket, s3_sample_key)
                            upload_to_s3(s3_client, mask_path, args.s3_bucket, s3_mask_key)
            ...
            if batch_idx % 10 == 0 and batch_idx > 0:
                ...
                try:
                    # Only delete temp files that have been processed
                    ...
                    # Only delete masks that have been processed
                ...
                # Clear the processed sets to avoid memory issues
                processed_files.clear()
                processed_masks.clear()
        ...
        print(f"Saving final results to {final_results_csv}...")
...
def main():
    ...
    os.makedirs(os.path.dirname(args.temp_results_csv), exist_ok=True)
    os.makedirs(os.path.dirname(args.final_results_csv), exist_ok=True)
    
    # Set up paths
    results_csv = os.path.join(args.temp_results_csv, "processing_results.csv")
    final_results_csv = os.path.join(args.final_results_csv, "processing_results.csv")
    
    # Process images
    process_images(args.csv_path, args.results_dir, results_csv, final_results_csv, args)
